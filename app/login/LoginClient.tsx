  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-500 font-sans ${isDarkMode ? 'bg-void text-slate-100' : 'bg-lightBg text-slate-900'}`} dir={lang === 'AR' ? 'rtl' : 'ltr'}>
      ...
      <footer className="w-full p-6 text-center text-xs md:text-sm text-[var(--nc-text-dim)] font-medium flex items-center justify-center gap-4 z-10 relative">
        <Link href="/privacy-policy" prefetch={false} className={`transition-colors ${isDarkMode ? 'hover:text-[var(--nc-foreground-muted)] font-medium' : 'hover:text-slate-800'}`}>
          {lang === 'AR' ? 'سياسة الخصوصية والأمان' : 'Privacy & Security Policy'}
        </Link>
        <span>|</span>
        <Link href="/disclaimer" prefetch={false} className={`transition-colors ${isDarkMode ? 'hover:text-[var(--nc-foreground-muted)] font-medium' : 'hover:text-slate-800'}`}>
          {lang === 'AR' ? 'إخلاء المسؤولية' : 'Disclaimer'}
        </Link>
        <span>|</span>
        <Link href="/terms-and-conditions" prefetch={false} className={`transition-colors ${isDarkMode ? 'hover:text-[var(--nc-foreground-muted)] font-medium' : 'hover:text-slate-800'}`}>
