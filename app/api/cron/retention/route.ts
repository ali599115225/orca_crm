...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await rawPrisma.$executeRaw`
  ...
  try {
    ...
    const result = await rawPrisma.$executeRaw`
      DELETE FROM sentinel_task_orders
  ...
  try {
    ...
    const result = await rawPrisma.$executeRaw`
      DELETE FROM agent_telemetry_logs
      WHERE created_at < ${cutoff}
    `;
  ...
  try {
    const cutoff = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000);
    const result = await rawPrisma.$queryRaw<Array<{ count: bigint }>>`
