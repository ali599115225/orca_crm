  ...
  try {
    ...
    const contracts = await prisma.contract.findMany({
      ...
      include: {
        ...
        offer: {
          select: { id: true, status: true, price: true, linkedOpportunityId: true },
        },
        paymentPlan: true,
        invoices: {
          where: { type: "SALE" },
    ...
    const data = contracts.map((contract) => {
      ...
      const installments = (saleInvoice?.installments || []).map((installment) => {
        ...
        return {
          ...
          paidAmount,
          remainingAmount: Math.max(0, Number(installment.amountSar) - paidAmount),
      ...
      return {
        ...
        paymentPlan: contract.paymentPlan
          ? {
              ...
              activatedAt: contract.paymentPlan.activatedAt?.toISOString() || null,
              completedAt: contract.paymentPlan.completedAt?.toISOString() || null,
            }
          : null,
        invoice: saleInvoice
          ? {
    ...
    const message = error instanceof Error ? error.message : "تعذر جلب عقود البيع.";
