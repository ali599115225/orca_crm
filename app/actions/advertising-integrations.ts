  ...
  return {
    ...
    run<T>(operation: () => Promise<T>): Promise<T> {
      return runWithTenantContext(
        { tenantId: tenant.id, userId: verifiedSession.userId },
        operation,
      );
    },
  };
...
export async function getAdvertisingConnectionsAction() {
  try {
    const { tenant, run } = await getAuthorizedContext();

    const rows = await run(async () => await prisma.platformConnection.findMany({
      where: {
        tenantId: tenant.id,
        platform: {
          in: [...ALL_PLATFORMS],
...
export async function saveStandardAdvertisingConnectionAction(input: {
  ...
  try {
    const { tenant, session, run } = await getAuthorizedContext();
    ...
    const existing = await run(async () => await prisma.platformConnection.findUnique({
      where: {
        tenantId_platform: {
          tenantId: tenant.id,
    ...
    const connection = await run(async () => await prisma.platformConnection.upsert({
      where: {
        tenantId_platform: {
          tenantId: tenant.id,
          platform: input.platform,
    ...
    await run(async () => await prisma.auditLog.create({
      data: {
        tenantId: tenant.id,
