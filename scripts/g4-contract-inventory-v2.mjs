...
const TAB_ARRAY_PATTERN = /(?:const|let)\s+[A-Za-z_$][\w$]*[Tt]abs[A-Za-z_$\w]*\s*=\s*\[([\s\S]{0,6000}?)\]\s*(?:as\s+const)?/g;
...
function tabValues(content) {
  const found = [...matches(TAB_VALUE_PATTERN, content)];
...
for (const file of sourceFiles) {
  ...
  metadata.set(file.path, {
    ...
    directModels: unique(matches(PRISMA_PATTERN, file.content)),
...
for (const file of sourceFiles) {
  ...
  if (values.length > 0 || roleTabCount > 0 || /\bactiveTab\b|\bsetActiveTab\b/.test(file.content)) {
    tabs.push({
...
const inventory = { schemaVersion: 2, summary, duplicateIds, contracts: allContracts.sort((a, b) => a.id.localeCompare(b.id)) };
