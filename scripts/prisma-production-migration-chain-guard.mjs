...
const ALLOWED_PRODUCTION_MIGRATIONS = Object.freeze([
  ...
  "20260923_final_architecture_persistence_readiness",
]);
...
function canonicalTextGitBlobSha(buffer) {
  const normalized = buffer.toString("utf8").replace(/\r\n/g, "\n").replace(/\r/g, "\n");
  return gitBlobSha(Buffer.from(normalized, "utf8"));
}
...
export function validateMigrationChain(root = DEFAULT_ROOT) {
  ...
  for (const [name, expectedBlobSha] of Object.entries(QUARANTINED_MIGRATIONS)) {
    ...
    const actualBlobSha = canonicalTextGitBlobSha(readFileSync(evidencePath));
