...
vi.mock('server-only', () => ({}));
// Deterministic replacement for the real SSRF-protection DNS/IP resolver used
// by lib/saudi-trust-gate/index.ts. The real requirePublicProviderUrl() performs
// a live DNS/IP lookup (tested independently under tests/email-smtp-provider.test.ts
// and tests/revenue-integrity/ejar-e2e.test.ts); this file's own invariant is the
// gate's sandbox-URL detection in production, not network reachability, so the
// resolver is isolated at the module boundary rather than requiring live DNS.
vi.mock('@/lib/payments/providers/custom-payment', () => ({
  requirePublicProviderUrl: async (input: string) => {
