...
// Reauthored from SOURCE tests/whatsapp/provider-agnostic-closure.test.ts.
// WhatsAppView / SettingsIntegrationsHub / WhatsAppIntegrationSettings assertions
// removed: those TARGET clean-room page/component surfaces do not exist yet
// (PAGE work not started). Only the proven non-visual capability — provider
// credential encryption, tenant-context re-entry at every API boundary, and
// webhook tenancy derived from the encrypted connection rather than request
...
describe("WhatsApp provider-agnostic closure", () => {
  ...
  it("derives tenant context before WhatsApp mutations", () => {
