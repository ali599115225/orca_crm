...
const BINDING_BY_OPERATION: Readonly<Record<string, EvidenceBinding>> = {
  ...
  "EXEC-003-C25-O01": binding(PILOT, "@/app/actions/rentals", "getRentalContractsAction", "getRentalContractsAction", "STATIC", "DIRECT_BEHAVIORAL C25 reaches the tenant-scoped Server Action read after authorization", "DIRECT_BEHAVIORAL C25 has no Platform Owner bypass when the database denies", downstream("prismaMocks.rentalLeaseFindMany"), "active tenant user reaches rental contracts", "platform owner without tenant user is denied"),
};
...
export const EXEC_003_INACTIVE_USER_ENTRYPOINT_COVERAGE = [
  ...
  { testFile: ENTRYPOINT_MATRIX, testName: "INACTIVE_USER_ENTRY_POINT Server Action C25 denies before downstream", entryPointLocalName: "getRentalContractsAction", downstreamSymbol: "prismaMocks.rentalLeaseFindMany", coverage: ["SERVER_ACTION", "READ", "SENSITIVE_READ"] },
