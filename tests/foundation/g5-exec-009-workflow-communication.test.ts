...
import type { WorkflowActor } from "@/lib/workflow-communication/contracts";
...
class MemoryRepository implements WorkflowCommunicationRepository {
  ...
  async transaction<T>(work: (tx: WorkflowCommunicationTransaction) => Promise<T>): Promise<T> {
    ...
    const tx: WorkflowCommunicationTransaction = {
      async findLatestWorkflowVersion(t, w) {
        return self.versions
          .filter((entry) => entry.tenantId === t && entry.workflowId === w)
          .sort((a, b) => b.version - a.version)[0] ?? null;
      },
      async insertWorkflowVersion(value) { self.versions.push(value); },
      async findWorkflowVersion(t, id) {
        return self.versions.find((entry) => entry.tenantId === t && entry.id === id) ?? null;
      },
      async findRun(t, id) {
        const value = self.runs.get(id) ?? null;
        return value?.tenantId === t ? value : null;
      },
      async findRunByKey(t, key) {
        return [...self.runs.values()].find(
          (entry) => entry.tenantId === t && entry.idempotencyKeyHash === key,
        ) ?? null;
      },
      async insertRun(value) { self.runs.set(value.id, value); },
      async updateRun(value) { self.runs.set(value.id, value); },
      async insertAttempt(value) { self.attempts.push(value); },
      async insertEscalation(value) { self.escalations.push(value); },
      async findThread(t, channel, identityHash) {
        return [...self.threads.values()].find(
          (entry) => entry.tenantId === t && entry.channel === channel && entry.identityHash === identityHash,
        ) ?? null;
      },
      async findThreadById(t, id) {
        const value = self.threads.get(id) ?? null;
        return value?.tenantId === t ? value : null;
      },
      async insertThread(value) { self.threads.set(value.id, value); },
      async updateThread(value) { self.threads.set(value.id, value); },
      async findCommunicationEventByProviderHash(t, channel, providerIdentityHash) {
        return self.events.find(
          (entry) =>
            entry.tenantId === t &&
            entry.channel === channel &&
            entry.providerIdentityHash === providerIdentityHash,
        ) ?? null;
      },
      async insertCommunicationEvent(value) { self.events.push(value); },
      async latestConsent(t, threadId, purpose) {
        return self.consents
          .filter((entry) => entry.tenantId === t && entry.threadId === threadId && entry.purpose === purpose)
          .sort((a, b) => b.effectiveAt.getTime() - a.effectiveAt.getTime())[0] ?? null;
      },
      async insertConsent(value) { self.consents.push(value); },
    };
    return work(tx);
  }
}
...
describe("EXEC-009 — durable workflow truth", () => {
  ...
  it("uses bounded retry, no blind non-retriable retry, and explicit dead letter escalation", async () => {
    ...
    expect(repo.runs.get(retriable.value.id)?.state).toBe("DEAD_LETTER");
...
describe("EXEC-009 — approval and escalation authority", () => {
  it("requires independent exact-scope approval and replays approved state safely", async () => {
    const repo = new MemoryRepository();
    const service = new WorkflowCommunicationService(repo);
    const version = await publish(service, {
    ...
    await expect(
      service.approveWorkflowRun({ actor: actor(), runId: run.value.id }),
  ...
  it("fails closed for wrong tenant, wrong scope, expired or missing approval assignment", async () => {
    const repo = new MemoryRepository();
    const service = new WorkflowCommunicationService(repo);
    const version = await publish(service, {
      approvalRequired: true,
    ...
    const run = await service.startWorkflowRun({ actor: actor(), workflowVersionId: version.id, idempotencyKey: "denials", payload: {} });

    await expect(
      service.approveWorkflowRun({ actor: actor(approverId, { tenantId: tenantOther }), runId: run.value.id }),
    ).rejects.toThrow(/WORKFLOW_RUN_NOT_FOUND/);
    await expect(
      service.approveWorkflowRun({
        actor: actor(approverId, { assignments: [{
    ...
    await expect(
      service.approveWorkflowRun({ actor: actor(approverId, { assignments: [] }), runId: run.value.id }),
    ).rejects.toThrow(/APPROVAL_AUTHORITY_DENIED/);
  });
});

describe("EXEC-009 — communication identity and consent truth", () => {
  it("normalizes identity, keeps unknown sender unbound, deduplicates exact provider identity, and rejects conflicts", async () => {
    ...
    expect(first.thread.identityState).toBe("UNKNOWN");
    expect(first.thread.partyId).toBeNull();
    expect(duplicate.duplicate).toBe(true);
    expect(repo.events).toHaveLength(1);

    await expect(
      inbound(service, { content: "conflicting body" }),
    ).rejects.toThrow(/PROVIDER_IDENTITY_CONFLICT/);
  });

  it("fails closed on ambiguous sender binding and allows only one verified party identity", () => {
  ...
  it("separates tenant/channel thread identity and stores explicit direction/purpose/provider identity", async () => {
    ...
    expect(result.event).toMatchObject({
      ...
      direction: "INBOUND",
      purpose: "SERVICE",
      providerIdentity: "wamid.explicit",
    });
  });
  ...
  it("uses configurable retention and legal hold without deleting audit identity", async () => {
    ...
    const expired = await service.setRetention({
      tenantId,
      threadId: thread.id,
      retentionPolicyKey: "POLICY_MESSAGE_CONTENT",
    ...
    expect(repo.events.find((entry) => entry.id === event.id)?.providerIdentityHash).toBe(event.providerIdentityHash);
