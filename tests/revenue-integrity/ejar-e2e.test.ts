...
// ── Shared public HTTPS transport boundary mock ────────────────────────────────
// EJAR now routes both the Saudi Trust Gate's URL check (via
// lib/payments/providers/custom-payment.ts → lib/net/public-https.ts) and the
// runtime provider call (app/actions/ejar.ts → publicHttpsJsonRequest) through
// this shared module. Mock it directly so tests never perform real DNS/TLS.
...
/**
 * Treats the shared @/lib/net/public-https boundary as public/safe (no real
 * DNS/TLS) and gives publicHttpsJsonRequest a default successful provider
 * transport response. Individual tests override with mockResolvedValueOnce /
 * mockRejectedValueOnce as needed.
 */
function mockPublicHttpsBoundaryDefaults() {
...
function setupHappyPath() {
  ...
  mockPublicHttpsBoundaryDefaults();
...
describe("Ejar idempotency states", () => {
  ...
  it("RETRYING (past nextRetryAt, retries remain) → FAILED_RETRYABLE → proceeds to call", async () => {
    ...
    // Simulate successful API call via the shared transport
    mockPublicHttpsJsonRequest.mockResolvedValueOnce({
      ok: true,
      status: 200,
    ...
    expect(mockPublicHttpsJsonRequest).toHaveBeenCalled();
  });
...
describe("submitContractToEjarAction: input / gate / provider scenarios", () => {
  ...
  it("Provider API fails → RETRYING state, no commission created", async () => {
    mockPublicHttpsJsonRequest.mockRejectedValueOnce(new Error("Network timeout"));
  ...
  it("happy path → commission created ONLY after real ejarContractId", async () => {
    ...
    expect(mockPublicHttpsJsonRequest).toHaveBeenCalledWith(
    ...
    const transportRequest = mockPublicHttpsJsonRequest.mock.calls[0][0];
    expect(String(transportRequest.url)).toBe(`${HUB_EJAR_BASE_URL}/contracts/register`);
  ...
  it("provider returns no contractId → treated as failure, no commission", async () => {
    mockPublicHttpsJsonRequest.mockResolvedValueOnce({
      ok: true,
      status: 200,
      payload: { contractNumber: "CR-999" }, // no contractId!
    });
...
describe("submitContractToEjarAction: Hub-only fail-closed", () => {
  ...
  it("CONNECTED EJAR connection missing → fail-closed, no provider fetch, no commission", async () => {
    setupHubEjarConnection({ connection: null });
    const transportMock = stubProviderSuccess();
