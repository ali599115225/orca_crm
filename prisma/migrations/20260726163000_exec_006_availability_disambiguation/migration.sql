...
-- Corrective additive migration following strict authority/availability hardening.
...
BEGIN
  ...
  IF EXISTS (
    ...
    SELECT 1 FROM "rental_leases" AS rental_record
    WHERE rental_record."tenant_id" = p_tenant_id
      AND rental_record."unit_id" = p_unit_id
      AND upper(rental_record."status") NOT IN (
        'CANCELLED', 'CANCELED', 'EXPIRED', 'REJECTED', 'TERMINATED', 'VOID'
      )
      AND rental_record."start_date" <= p_now::DATE
      AND rental_record."end_date" >= p_now::DATE
  ) THEN
    RETURN QUERY SELECT
...
COMMENT ON FUNCTION "exec006_evaluate_unit_availability" IS
  'Computes fail-closed Unit availability with fully-qualified persisted sources after EXEC-006 authority hardening.';
