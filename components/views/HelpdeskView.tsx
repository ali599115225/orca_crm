...
const TRANSLATIONS = {
  AR: {
    title: "مركز الدعم والمستندات المشترك (Helpdesk & Docs)",
    subtitle: "إدارة الملفات والمستندات العقارية وتلقي الدعم الفني الفوري من المساعد الذكي.",
    tabDocs: "📂 مستودع الوثائق والمخططات",
    tabSupport: "💬 تذاكر الدعم والوكيل مساعد",
    badgeText: "مركز التحكم الموحد - Central Operations Hub",
  ...
  EN: {
    title: "Support & Documents Hub",
    subtitle: "Manage blueprints, contracts, and access 24/7 AI-driven helpdesk support.",
    tabDocs: "📂 Document Archive & Blueprints",
    tabSupport: "💬 Support Tickets & AI Agent",
    badgeText: "Central Operations Hub",
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="bg-[#1C2B48] border border-[#A7C7E7]/20 rounded-2xl p-6 shadow-sm space-y-4">
              <div className="border-b border-slate-100 dark:border-[#A7C7E7]/20 pb-3">
                <h3 className="text-[#E8ECEF] font-bold dark:text-white font-bold text-base">{t.openTicket}</h3>
                <p className="text-[11px] text-slate-450 dark:text-[#C4D8E5] font-medium mt-0.5">{t.openTicketSub}</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-[#C4D8E5] font-medium dark:text-[#C4D8E5] font-medium text-xs font-semibold mb-2">{t.subjectLabel}</label>
                  <input 
                    type="text" 
                    name="title" 
                    required 
                ...
                <div>
                  <label className="block text-[#C4D8E5] font-medium dark:text-[#C4D8E5] font-medium text-xs font-semibold mb-2">{t.detailsLabel}</label>
                  <textarea 
                    name="description" 
                    rows={4} 
                    required 
                ...
                <button 
                  type="submit" 
                  disabled={loading}
                  className="w-full py-3 rounded-xl bg-[#8EB1D1] hover:bg-[#A7C7E7] text-white font-bold text-sm transition-colors cursor-pointer disabled:opacity-55 hover:shadow-sm"
                >
            ...
            {/* Ticket list ledger */}
            <div className="bg-[#1C2B48] border border-[#A7C7E7]/20 rounded-2xl p-6 shadow-sm space-y-4">
              <h4 className="text-[#E8ECEF] font-bold dark:text-white font-bold text-sm border-b border-slate-100 dark:border-[#A7C7E7]/20 pb-3 flex justify-between items-center">
                <span>{t.ledgerTitle}</span>
                <span className="text-xs text-[#C4D8E5] font-medium font-normal">{t.totalTickets.replace('{count}', toArabicNumerals(tickets.length))}</span>
              </h4>

              <div className="space-y-2 max-h-[300px] overflow-y-auto scrollbar-fade">
                {tickets.length === 0 ? (
                  <div className="py-6 text-center text-[#C4D8E5] font-medium dark:text-[#C4D8E5] font-medium text-xs">
                    {t.emptyStateLeft}
                  </div>
                ) : (
                  tickets.map((ticket) => {
                    ...
                    return (
                      <div 
                        ...
                        className={`p-3 rounded-xl border transition-all cursor-pointer flex justify-between items-start ${
                          isActive 
                            ? 'bg-[#8EB1D1]/5 border-[#8EB1D1]/40' 
                            : 'bg-slate-50 dark:bg-[#1C2B48] border-[#A7C7E7]/20 dark:border-slate-850 hover:border-slate-350 dark:hover:border-[#A7C7E7]/20'
                        }`}
                      >
                        <div className="space-y-1">
                          <h5 className="font-bold text-xs text-[#E8ECEF] font-bold dark:text-white line-clamp-1">{ticket.title}</h5>
                          <p className="text-[10px] text-[#C4D8E5] font-medium">{t.ticketDate.replace('{date}', formatTicketDate(ticket.createdAt))}</p>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          ...
                          <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${
                            isOpen 
                              ...
                              : 'bg-slate-200/80 dark:bg-[#1C2B48] text-[#C4D8E5] font-medium border-slate-250 dark:border-slate-700'
                          }`}>
          ...
          <div className="lg:col-span-7">
            {selectedTicket ? (
              <div className="bg-[#1C2B48] border border-[#A7C7E7]/20 rounded-2xl p-6 shadow-sm space-y-6">
                ...
                {/* Detail header */}
                <div className="border-b border-slate-100 dark:border-[#A7C7E7]/20 pb-4 flex justify-between items-center gap-4">
                  <div>
                    <h3 className="text-[#E8ECEF] font-bold dark:text-white font-bold text-lg">{selectedTicket.title}</h3>
                    <p className="text-[10px] text-slate-455 dark:text-[#C4D8E5] font-medium mt-1 font-en">
                ...
                {selectedTicket.status === 'OPEN' && (
                  <div className="flex justify-between items-center bg-indigo-500/10 border border-indigo-500/20 px-4 py-2.5 rounded-xl text-xs">
                    <span className="font-bold text-indigo-400">{t.slaLabel}</span>
                    <span className="font-black text-indigo-500 font-en">
                ...
                {/* Inquiry description */}
                <div className="bg-slate-50 dark:bg-[#1C2B48] border border-slate-100 dark:border-slate-850 p-4 rounded-xl">
                  <p className="text-[#8EB1D1] text-[10px] font-bold mb-1.5">{t.authorLabel}</p>
                  <p className="text-slate-750 dark:text-[#C4D8E5] font-medium text-xs leading-relaxed whitespace-pre-wrap">{selectedTicket.description}</p>
                </div>
                ...
                {selectedTicket.aiResponse ? (
                  <div className="bg-indigo-500/5 border border-indigo-500/20 p-5 rounded-2xl space-y-3 relative overflow-hidden">
                    ...
                    <div className="text-slate-700 dark:text-[#C4D8E5] font-medium text-xs leading-relaxed whitespace-pre-wrap relative z-10">
                      {selectedTicket.aiResponse}
                    </div>
                  </div>
                ) : null}
                ...
                <div className="space-y-4">
                  <h4 className="text-[#E8ECEF] font-bold dark:text-white font-bold text-xs border-b border-slate-100 dark:border-slate-850 pb-2">
                    ...
                    {replies.map(rep => {
                      ...
                      return (
                        <div key={rep.id} className={`flex flex-col ${isClient ? 'items-end' : 'items-start'} space-y-1`}>
                          <div className={`p-3 rounded-xl text-xs leading-normal max-w-[85%] ${isClient ? 'bg-[#8EB1D1] text-white rounded-br-none' : 'bg-slate-100 dark:bg-[#1C2B48] text-[#E8ECEF] font-bold dark:text-slate-200 rounded-bl-none'}`}>
                            {rep.message}
                          </div>
                          <span className="text-[9px] text-[#C4D8E5] font-medium font-en px-1">{new Date(rep.createdAt).toLocaleTimeString()}</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  {selectedTicket.status === 'OPEN' && (
                    <form onSubmit={handleSendReply} className="flex gap-2">
                      <input
                        ...
                        className="flex-grow rounded-xl bg-slate-50 dark:bg-[#1C2B48] border border-[#A7C7E7]/20 dark:border-slate-850 px-4 py-2.5 text-xs text-[#E8ECEF] font-bold dark:text-white placeholder-slate-400 focus:outline-none focus:border-[#8EB1D1]"
                      />
                      <button
                        type="submit"
                        disabled={submittingReply || !replyInput.trim()}
                        className="bg-[#8EB1D1] hover:bg-[#A7C7E7] text-white px-4 py-2.5 rounded-xl font-bold text-xs cursor-pointer transition-colors disabled:opacity-50 shrink-0"
                      >
              ...
              <div className="bg-[#1C2B48] border border-[#A7C7E7]/20 rounded-2xl p-16 text-center shadow-sm">
                <i className="ph ph-article-ny-times text-5xl text-[#C4D8E5] font-medium dark:text-[#C4D8E5] font-medium mb-4 block"></i>
                <h3 className="text-[#E8ECEF] font-bold dark:text-white font-bold text-base mb-1">{t.emptyStateRightTitle}</h3>
                <p className="text-[#C4D8E5] font-medium text-xs">{t.emptyStateRightDesc}</p>
              </div>
            )}
          </div>
        </div>
