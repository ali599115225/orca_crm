...
import { Calendar, Search, Eye, FileText, X } from 'lucide-react';
...
import { getToursAction } from '@/app/actions/tours';
...
export default function ToursView() {
  ...
  const handleRowClick = (tour: TourListItem) => {
  ...
  const columns: Column<TourListItem>[] = [
    {
      header: 'الموعد',
      accessor: (t) => <span className="font-bold">{formatDateTime(t.startAt)}</span>,
    },
    {
      header: 'العميل',
      accessor: 'leadName',
  ...
  return (
    <div dir="rtl" className="space-y-6">
      ...
      >
        <button
          disabled
          onClick={() => toast.info('سيتم تفعيل جدولة الجولات لاحقًا.')}
          className="nc-btn nc-btn-primary text-xs font-bold opacity-50 cursor-not-allowed"
        >
          <Calendar size={16} />
          جدولة جولة
        </button>
      ...
      <LayoutContainer
        kpis={null}
        actions={
          <SmartCard elevation="default" className="p-4 space-y-3">
            <p className="text-xs font-bold text-[var(--nc-text-primary)]">فلاتر البحث</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <input
                ...
                className="w-full rounded-lg bg-[var(--nc-surface-strong)] border border-[var(--nc-border)] text-[var(--nc-text-primary)] text-[11px] p-2 outline-none focus:border-[var(--nc-accent-border)]"
            ...
            <div className="flex gap-2">
              <button onClick={handleSearch} className="flex-1 nc-btn nc-btn-primary nc-btn-sm text-[11px] font-bold">
                تطبيق
              </button>
              <button onClick={handleClearFilters} className="flex-1 nc-btn nc-btn-ghost nc-btn-sm text-[11px] font-bold">
                مسح
              </button>
            </div>
          </SmartCard>
