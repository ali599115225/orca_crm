...
export default function Opportunities() {
  ...
  const [leadId, setLeadId] = useState(""); const [value, setValue] = useState(""); const [probability, setProbability] = useState("50"); const [closeDate, setCloseDate] = useState(""); const [unitId, setUnitId] = useState("");
  ...
  const handleCreateOpportunity = async (e: React.FormEvent) => { e.preventDefault(); if (!leadId || !value) return; try { setBtnLoading(true); await fetch("/api/v1/opportunities", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ leadId, value, probability, closeDate, unitId: unitId || null }) }); setLeadId(""); setValue(""); setProbability("50"); setCloseDate(""); setUnitId(""); loadData(); } catch (err) { console.error(err); } finally { setBtnLoading(false); } };
