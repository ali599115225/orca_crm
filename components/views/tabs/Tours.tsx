...
export default function Tours() {
  ...
  const [location, setLocation] = useState("مشروع النرجس السكني");
  ...
  const loadData = async () => {
    try {
      ...
      if (toursJson.success) setTours(toursJson.data);
  ...
  const handleCreateTour = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!leadId || !startAt || !location) return;
  ...
  const handleUpdateStatus = async (tourId: string, toStatus: string) => {
    try {
      setBtnLoading(true);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <SmartCard className="p-4">
          ...
          <form onSubmit={handleCreateTour} className="space-y-4 text-xs">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[var(--nc-text-dim)] font-medium">تاريخ ووقت المعاينة الميدانية *</label>
              <input
                type="datetime-local"
                value={startAt}
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[var(--nc-text-dim)] font-medium">موقع الجولة / المشروع *</label>
              <select
                value={location}
              ...
              >
                ...
                <option value="برج أوركا الياسمين">برج أوركا الياسمين</option>
                <option value="مشروع أبراج مكة الإدارية">مشروع أبراج مكة الإدارية</option>
                <option value="جولة افتراضية Virtual Tour">جولة افتراضية Virtual Tour</option>
              </select>
            </div>

            <div className="flex flex-col gap-1">
              <label className="text-[var(--nc-text-dim)] font-medium">عدد المرافقين</label>
              <input
                type="number"
                value={attendees}
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[var(--nc-text-dim)] font-medium">ملاحظات إضافية (أية تفاصيل خاصة)</label>
              <textarea
        ...
        <SmartCard className="lg:col-span-2 p-4">
          ...
          {loading ? (
            ...
            <div className="py-12 text-center text-[var(--nc-text-dim)] font-medium text-xs">لا توجد جولات عقارية مجدولة حالياً.</div>
          ) : (
            <div className="space-y-3.5 max-h-[440px] overflow-y-auto pr-1 scrollbar-fade">
              {tours.map((tour) => {
                const dateStr = new Date(tour.startAt).toLocaleString("ar-SA", {
                  dateStyle: "medium",
                  timeStyle: "short",
                });
                return (
                  <div key={tour.id} className="p-4 bg-[var(--nc-surface-solid)] border border-[var(--nc-glass-border)] rounded-xl hover:border-[var(--nc-accent-border)]/40 transition-colors flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div className="space-y-1 text-xs">
                      ...
                      <p className="text-slate-450">موقع الجولة: <span className="text-[var(--nc-text-dim)] font-medium font-semibold">{tour.location}</span></p>
                      <p className="text-slate-450 font-en">التاريخ: <span className="text-indigo-400 font-semibold">{dateStr}</span></p>
                      {tour.notes && <p className="text-[var(--nc-text-dim)] font-medium italic text-xs">ملاحظة: {tour.notes}</p>}
                    </div>
                    ...
                    {tour.status === "SCHEDULED" && (
                      <div className="flex gap-2">
                        <button
                          ...
                          disabled={btnLoading}
                          className="bg-emerald-500/10 hover:bg-emerald-500/25 text-emerald-400 border border-emerald-500/30 px-3 py-1 rounded text-xs transition-colors"
                        >
                          إتمام الزيارة
                        </button>
                        <button
                          ...
                          disabled={btnLoading}
                          className="bg-rose-500/10 hover:bg-rose-500/25 text-rose-400 border border-rose-500/30 px-3 py-1 rounded text-xs transition-colors"
                        >
                          إلغاء
                        </button>
                      </div>
